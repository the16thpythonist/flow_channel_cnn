"""
This experiment module implements the training of an InvariantCNN model for the prediction of the laminar flow 
...
to construct an ``InvariantCNN`` model using the specified parameters. This model implements a CNN architecture 
that is invariant to vertical flipping and horizontal shifting of the input data.
"""
...
def construct_model(e: Experiment):
    ...
    This implementation constructs an ``InvariantCNN`` model instance with the specified parameters.
    """
    e.log('Constructing InvariantCNN model...')
    model = InvariantCNN(
        input_dim=1,
