"""
This experiment module implements the base experiment for the training of a CNN model for the 
prediction of the laminar flow channel parameters $C_f$ and $S_t$ based on a dataset of flow 
channel geometry images.

Note that this base experiment does not implement an actual model but only defines a model of
the ``MockCNN`` type as a placeholder as the return of the ``construct_model`` hook which 
needs to be overwritten in a concrete sub experiment.
"""
import os
import random
...
@experiment.hook('construct_model', default=True, replace=False)
def construct_model(e: Experiment) -> AbstractCNN:
    """
    ...
    This default implementation constructs a ``MockCNN`` model which only serves as a placeholder for the actual
    model implementations that should be implemented in sub-experiments.
    """
    
    e.log('NOTE: USING MOCK CNN MODEL!')
    model = MockCNN(
        input_dim=1,
...
def experiment(e: Experiment):
    ...
    # :hook load_dataset:
    #       This hook is supposed to load the dataset and return it as a tuple (train, val, test) of lists where the
    #       first element is a list containing tuples (x, y) of the training set samples and the second element is a
    #       list containing elements (x, y) of the test set samples.
    train, val, test = e.apply_hook(
        'load_dataset'
    )
    ...
    # :hook construct_model:
    #       This hook is supposed to construct the model instance which can then subsequently be trained with the
    #       flow channel dataset. The model that is returned needs to implement the AbstractCNN interface.
    model: AbstractCNN = e.apply_hook(
        'construct_model',
    )
    e.log('model summary:')
    e.log(str(model))
